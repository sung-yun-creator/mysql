...
interface WdogInputDateTermStateProps {
  title?: string;
  from_dt?: Date;
  to_dt?: Date;
  onDateChange?: (dateRange: DateRange) => void;  // 콜백 추가  
}

const today = new Date();

const WdogInputDateTermState = ({
  title = '기간',
  from_dt = startOfMonth(today),
  to_dt = endOfMonth(today),
  onDateChange,
}: WdogInputDateTermStateProps) => {
  ...
  const handleTempSelect = (selected: DateRange | undefined) => {
    ...
    const currentMonthEnd = addDays(addMonths(selected.from, 1), -1);
    // to가 from 한달 뒤 날짜면 한달뒤로 제한 
    if (selected.to && selected.to >= currentMonthEnd) {
      selected.to = currentMonthEnd;
    }
    setTempDate(selected);
    if (selected.to) {
      onDateChange?.(selected);  // 부모에게 알림
    }
  };
    ...
    <Popover open={open} onOpenChange={setOpen}>
        ...
        <div className="flex items-center gap-2 w-full h-10">  {/* flex 컨테이너 */}
          ...
          <div className="flex-1 min-w-55 relative">
            <Input
              ...
              className="h-10 pr-10 pl-2 text-sm border rounded-md focus-visible:ring-2 bg-control"
            />
      ...
      <PopoverContent className="w-auto p-0" align="start">
        ...
        <div className="flex items-center justify-end space-x-2 p-3 border-t">
          <div className="text-primary">최대 1달 범위로 선택해주세요 </div>
          <Button
            type="button"
            variant="outline"
            size="sm"
...
export default WdogInputDateTermState;
